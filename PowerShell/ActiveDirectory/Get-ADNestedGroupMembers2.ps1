...
<#  
JQ - Added logic to skip groups that do not exist.
...
$modules = get-module | select -expand name
    ...
    { 
		...
		if ($ADGroupname)
		{
            ...
            # NOT NULL
			write-output "ADGroupname is NOT null."
		
    		
    		
    		
    		
    		write-output "ADGroupname: $($ADGroupname)"
    		
    		$memberof = $adgroupname | select -expand memberof 
    		
    		write-output "memberof: $($memberof)"
    		
    		
    	
    		write-output "Checking group: $($adgroupname.name)" 
